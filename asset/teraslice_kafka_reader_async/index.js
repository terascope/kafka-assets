...
function newReader(context, opConfig) {
    ...
    function createConsumer() {
        ...
        if (opConfig.debug) options.rdkafka_options.debug = opConfig.debug;
        return context.foundation.getConnection(options).client;
    }
    ...
    function processSlice(data) {
        return new Promise((resolveSlice, rejectSlice) => {
            ...
            function error(err) {
                logger.error('an error occured in the kafka client', err);
                clearPrimaryListeners();
                rejectSlice(err);
            }
                    ...
                    consumer.consume(opConfig.size - slice.length, (err, messages) => {
                        if (err) {
                            if (!(errorDict[err.code] || errorDict[err])) {
                                logger.error('an error occured in while consuming', err);
                                readyToProcess = true;
                                clearPrimaryListeners();
                                rejectSlice(err);
                                return;
