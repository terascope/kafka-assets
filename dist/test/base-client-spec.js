...
const base_client_1 = __importDefault(require("../asset/src/_kafka_clients/base-client"));
const codes = __importStar(require("../asset/src/_kafka_helpers/error-codes"));
describe('Base Client (internal)', () => {
    const logger = (0, job_components_1.debugLogger)('base-client');
    class FakeKafkaClient extends events_1.EventEmitter {
        constructor() {
            super(...arguments);
            this.connected = true;
        }
    ...
    beforeEach(() => {
        ...
        base_client_1.default.DEFAULT_BACKOFF = 10;
        base_client_1.default.DEFAULT_MAX_RETRIES = 2;
        base_client_1.default.BACKOFF_RANDOM_FACTOR = [0, 1];
        // @ts-expect-error
        client = new base_client_1.default(fakeClient, 'test-topic', logger);
    ...
    describe('->disconnect', () => {
        ...
        it('should handle an error on disconnect', async () => {
            ...
            fakeClient.disconnect = jest.fn((cb) => {
            ...
            try {
                ...
                expect(err).toHaveProperty('message', 'Failed to disconnect, caused by error: Disconnect error');
            }
    ...
    describe('->_connect', () => {
        ...
        it('should handle an error on connect', async () => {
            fakeClient.connect = jest.fn((obj, cb) => {
            ...
            try {
                ...
                expect(err).toHaveProperty('message', 'Failed to connect, caused by error: Connect error');
            }
        });
    });
    describe('->_once', () => {
    ...
    describe('_try', () => {
        ...
        describe('when it succeeds on the second attempt', () => {
            it('should call the fn twice', async () => {
                const error = new Error('ERR__TIMED_OUT');
                error.code = codes.ERR__TIMED_OUT;
        ...
        describe('when it throws an okay error on the second attempt', () => {
            it('should call the fn twice', async () => {
                const error = new Error('ERR__TIMED_OUT');
                error.code = codes.ERR__TIMED_OUT;
                const okError = new Error('KAFKA_NO_OFFSET_STORED');
        ...
        describe('when it succeeds on the third attempt', () => {
            it('should call the fn thrice', async () => {
                const error = new Error('ERR__RESOLVE');
        ...
        describe('when it fails on the third attempt when a fatal error', () => {
            it('should call the fn thrice and throw', async () => {
                ...
                const retryable = new Error('Uh oh');
        ...
        describe('when it fails on the third attempt when a retryable error', () => {
            it('should call the fn thrice and throw', async () => {
                expect.hasAssertions();
                const error = new Error('ERR__WAIT_CACHE');
